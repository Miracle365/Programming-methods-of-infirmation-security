...
void point_constr(struct Point *point){
    ...
    BN_hex2bn(&p, P_PARAMETR);          // module
    BN_hex2bn(&v, V_PARAMETER);        // v parameter of Twisted Edwards
    BN_dec2bn(&g, "1");                   // some "one"
    BN_dec2bn(&point->z, "1");            // another "one"
    BN_hex2bn(&u, U_PARAMETER);
    ...
    BN_mod_inverse(a, u, p, tmp);
    if(!point->y) point->y = BN_new();
    BN_mod_mul(point->y, a, point->x, p, tmp);


    BN_free(a); BN_free(p); BN_free(v); BN_free(g); BN_free(f); BN_CTX_free(tmp); BN_free(u);
}
